  ...
  private buildPrompt(input: GeminiMappingInput): string {
    ...
    const groupSummaries = groupEntries.map(([group, candidates]) => {
      const entries = candidates
        ...
        .join(", ");
      return `${group.toUpperCase()} (DEPRECATED): ${entries}`;
    });

    const candidateLines = groupEntries.flatMap(([group, candidates]) => {
      const header = `Group label (deprecated): ${group}`;
      ...
      const examples = Array.isArray(candidate.examples) && candidate.examples.length > 0
        ...
        : "None listed";
      return `- ${candidate.id} (${candidate.name}) - ${descriptionSnippet} | Examples: ${examples}`;
      });
      return [header, ...lines];
    });
...
Select selected:true only when vendor documentation explicitly states the product logs/records/emits telemetry for that behavior.
Only choose IDs from the candidate list.
Return ONLY valid JSON (no markdown, no code fences, no commentary).
Return JSON only in this shape:
{
  "decisions": [
    { "id": "DC0001", "selected": true, "confidence": "high", "scope": "exact", "reason": "Short justification", "evidence": "Quote or paraphrase", "source_url": "https://..." },
...
- If the product platform(s) do not include the candidate's platform context (if provided), default to "selected": false unless evidence explicitly shows cross-platform coverage.
- Group headings are for readability only; base selected decisions solely on evidence that the product logs the specific Data Component behavior.
- If group is UNKNOWN, treat as "parent label missing"; do not penalize selection; decide per-DC from evidence.
- If you cannot find an authoritative vendor source URL that supports the DC, set "selected": false.
- Evidence must explicitly mention the exact product name or one of its aliases as listed above. If evidence only refers to the vendor/platform/suite without explicitly listing this product, set "selected": false.
- For "selected": false, omit "confidence", "reason", "evidence", and "source_url".
- Do not fabricate URLs or quotes; source_url must be a real page and the evidence must directly support that the product logs the DC's described activity.
- For "selected": true, include a grounded reason, evidence, a source_url from the search results, confidence (high|medium|low), and scope ("exact" if the product name is explicitly stated; "suite-explicit" if the documentation explicitly lists this product among supported modules; "platform-explicit" only if the documentation explicitly states platform-wide coverage including this product). Confidence and scope are required for selected:true.
